...
function installSkills(options = {}) {
  ...
  for (const providerId of selectedProviders) {
    ...
    for (const currentScope of targetScopes) {
      ...
      // Also copy rules into the harness rules directory for direct rule loading
      const rulesSrcDir = path.join(packageRoot, 'rules');
      if (fs.existsSync(rulesSrcDir)) {
        let rulesTargetDir = null;
        if (providerId === 'antigravity') {
          rulesTargetDir = currentScope === 'project'
        ...
        if (rulesTargetDir) {
          copyDirRecursive(rulesSrcDir, rulesTargetDir);
        }
      }
